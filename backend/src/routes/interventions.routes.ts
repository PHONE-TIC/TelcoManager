...
import { authenticate, requireGestionnaireOrAdmin, requireTechnicienOrAdmin } from '../middleware/auth.middleware';
...
router.get('/:id', param('id').isUUID(), interventionController.getInterventionById);
...
router.post(
    ...
    [
        ...
        body('statut').optional().isIn(['planifiee', 'en_cours', 'terminee', 'annulee']),
    ],
...
router.delete('/:id', requireGestionnaireOrAdmin, param('id').isUUID(), interventionController.deleteIntervention);
...
router.post('/:id/lock', requireTechnicienOrAdmin, param('id').isUUID(), interventionController.lockIntervention);
router.post('/:id/unlock', requireTechnicienOrAdmin, param('id').isUUID(), interventionController.unlockIntervention);
