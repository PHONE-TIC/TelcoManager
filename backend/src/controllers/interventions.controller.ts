...
export const getAllInterventions = async (req: AuthRequest, res: Response) => {
    try {
        const { clientId, technicienId, statut, startDate, endDate, page = '1', limit = '20' } = req.query;
...
export const createIntervention = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (req.user?.role !== 'admin') {
            return res.status(403).json({ error: 'Seuls les administrateurs peuvent créer des interventions' });
        }
        ...
        const finalNumero = `RDV${year}${initialIntervention.compteur.toString().padStart(3, '0')}`;
...
export const updateIntervention = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (req.user?.role === 'technicien') {
            if (existingIntervention.statut === 'terminee' || existingIntervention.statut === 'annulee') {
                return res.status(403).json({ error: 'Les interventions clôturées ne peuvent pas être modifiées' });
            }
        }
...
export const updateInterventionStatus = async (req: AuthRequest, res: Response) => {
    try {
        const { id } = req.params;
        ...
        if (statut === 'en_cours' && existingIntervention.statut === 'planifiee') {
            ...
            if (scheduledDate < today || scheduledDate >= tomorrow) {
                return res.status(400).json({
                    error: 'Impossible de prendre en charge une intervention qui n\'est pas prévue pour aujourd\'hui'
                });
            }
        }
...
export const validateHours = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (!heureArrivee || !heureDepart) {
            return res.status(400).json({ error: 'Heures d\'arrivée et de départ requises' });
        }
...
export const manageEquipement = async (req: AuthRequest, res: Response) => {
    try {
        const { id } = req.params;
        const { stockId, action, quantite = 1, notes, etat, nom, marque, modele, serialNumber } = req.body;
        // action: 'install', 'retrait'
        // etat (si retrait): 'ok', 'hs'

        // Validation for stockId OR manual details
        if (!stockId && !nom) {
            return res.status(400).json({ error: 'stockId ou nom du matériel requis' });
        }
        ...
        if (action === 'install') {
            if (!stockId) return res.status(400).json({ error: 'stockId requis pour installation' });
            ...
            if (intervention.technicienId) {
                try {
                    ...
                    if (!techStock || techStock.quantite < quantite) {
                        ...
                        return res.status(400).json({ error: 'Stock insuffisant dans le véhicule' });
                    }
...
export const lockIntervention = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (intervention.lockedBy && intervention.lockedBy !== userId && intervention.lockedAt) {
            const fiveMinutesAgo = new Date(Date.now() - 5 * 60 * 1000);
            if (intervention.lockedAt > fiveMinutesAgo) {
                // Get user name if possible
                const lockingUser = await prisma.technicien.findUnique({ where: { id: intervention.lockedBy } });
