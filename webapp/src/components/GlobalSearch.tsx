  ...
  const getAllResults = () => {
    ...
    if (activeTab === "all" || activeTab === "clients") {
      results.clients.forEach((item) =>
        allResults.push({ type: "client", data: item })
      );
    }
    if (activeTab === "all" || activeTab === "interventions") {
      results.interventions.forEach((item) =>
        allResults.push({ type: "intervention", data: item })
      );
    }
    if (activeTab === "all" || activeTab === "stock") {
      results.stock.forEach((item) =>
        allResults.push({ type: "stock", data: item })
      );
    }
    if (activeTab === "all" || activeTab === "techniciens") {
      results.techniciens.forEach((item) =>
          ...
          {type === "intervention" && (
            <>
              ...
              <div className="result-subtitle">
                {data.client?.nom} •{" "}
                {new Date(data.datePlanifiee).toLocaleDateString()}
              </div>
            </>
          )}
          {type === "stock" && (
            <>
              <div className="result-title">
                {highlightText(data.nomMateriel, query)}
              </div>
              <div className="result-subtitle">
                Réf: {data.reference} • N° Série: {data.numeroSerie} • Qté:{" "}
                {data.quantite}
              </div>
            </>
          )}
    ...
    >
      <div className="global-search-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="search-tabs">
          ...
          >
            Interventions{" "}
            <span className="tab-count">{results.interventions.length}</span>
          </button>
          <button
