...
export const generateInventoryPDF = (session: any) => {
    ...
    doc.text(`Statut: ${session.status === 'completed' ? 'Finalisé' : 'Brouillon'}`, 14, 40);
    ...
    autoTable(doc, {
        startY: 55,
        head: [['Matériel', 'Référence', 'Code-barres', 'Qte Théorique', 'Qte Comptée', 'Notes']],
        body: tableBody,
        theme: 'grid',
        ...
        didDrawCell: (data) => {
            if (data.section === 'body' && data.column.index === 2) {
                ...
                if (barcodeUrl && typeof barcodeUrl === 'string' && barcodeUrl.startsWith('data:image')) {
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        doc.setFontSize(8);
        doc.text(
            `Page ${i} / ${totalPages} - Généré le ${moment().format('DD/MM/YYYY HH:mm')}`,
            pageWidth / 2,
