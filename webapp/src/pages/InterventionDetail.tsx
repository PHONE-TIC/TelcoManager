...
const InterventionDetail: React.FC = () => {
        ...
        try {
            ...
            if (!isEditing) { // Don't overwrite if editing
                setFormData({
                    ...
                    datePlanifiee: data.datePlanifiee ? formatDateTimeLocal(data.datePlanifiee) : '',
                    statut: data.statut,
                    notes: data.notes || '',
    ...
    const handleCancel = () => {
        if (intervention) {
            setFormData({
                ...
                datePlanifiee: intervention.datePlanifiee ? formatDateTimeLocal(intervention.datePlanifiee) : '',
    ...
    return (
        <div className="page-container intervention-detail">
            ...
            {user?.role === 'technicien' && !isClosedIntervention(intervention.statut) && (
                ...
                <div className="info-grid">
                    ...
                    {user?.role === 'admin' && intervention.client.rue && (
                        <div className="info-item">
                            <label>Adresse</label>
                            <div className="info-value">
                                {intervention.client.rue}<br />
                                <small>{intervention.client.codePostal} {intervention.client.ville}</small>
                            </div>
                        </div>
                    )}
                            ...
                            <div className="info-value">
                                {user?.role === 'admin' && !isClosedIntervention(intervention.statut) ? (
                                    ...
                                    >
                                        ...
                                        {techniciens.filter(t => t.role !== 'admin').map((tech) => (
            ...
            <div className="info-card" style={{ borderLeft: '4px solid var(--success-color, #10b981)' }}>
            ...
            {user?.role === 'admin' && (
                <div className="info-card" style={{ borderLeft: '4px solid var(--primary-color, #3b82f6)' }}>
                    <h3>📜 Historique</h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            ...
                            <div>
                                <strong style={{ color: 'var(--text-primary)' }}>Création</strong>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            ...
                            <div>
                                <strong style={{ color: 'var(--text-primary)' }}>Rendez-vous prévu</strong>
                                <div style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
                                    {new Date(intervention.datePlanifiee).toLocaleString('fr-FR')}
                                </div>
                            </div>
                        ...
                        {intervention.heureArrivee && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                ...
                                <div>
                                    <strong style={{ color: 'var(--text-primary)' }}>Arrivée sur site</strong>
                                    <div style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
                                        {new Date(intervention.heureArrivee).toLocaleString('fr-FR')}
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        {intervention.heureDepart && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                ...
                                <div>
                                    <strong style={{ color: 'var(--text-primary)' }}>Départ du site</strong>
                                    <div style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
                        ...
                        {intervention.dateRealisee && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                ...
                                <div>
                                    <strong style={{ color: 'var(--text-primary)' }}>Terminée</strong>
                                    <div style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
                                        {new Date(intervention.dateRealisee).toLocaleString('fr-FR')}
                                    </div>
                                </div>
                        ...
                        {intervention.statut === 'annulee' && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                ...
                                <div>
                                    <strong style={{ color: 'var(--text-primary)' }}>Annulée</strong>
                                </div>
                            </div>
                        )}
                    </div>
            ...
            <PhotoCapture
                ...
                readOnly={intervention.statut === 'terminee' || intervention.statut === 'annulee'}
            />
        </div>
    );
};
