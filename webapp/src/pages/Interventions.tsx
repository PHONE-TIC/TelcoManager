...
function Interventions() {
    ...
    const { user } = useAuth();
    const { isOnline, cacheInterventionsList, getCachedInterventionsList, pendingSyncCount } = useOffline();
    ...
    const [viewMode, setViewMode] = useState<'list' | 'calendar' | 'all'>(initialViewMode);
    ...
    const getStatusBadge = (statut: string) => {
        const badges: { [key: string]: { label: string; class: string } } = {
            ...
            en_cours: { label: '⏳ En cours', class: 'bg-yellow-100 text-yellow-800' },
            terminee: { label: '✓ Terminée', class: 'bg-green-100 text-green-800' },
            annulee: { label: '✕ Annulée', class: 'bg-red-50 text-red-600' },
        };
        const badge = badges[statut] || { label: statut, class: 'bg-gray-100 text-gray-600' };
    ...
    const getPriorityIndicator = (datePlanifiee: string, statut: string) => {
        ...
        if (date < now && statut !== 'terminee') {
            return <span title="En retard" style={{ display: 'inline-block', width: 10, height: 10, borderRadius: '50%', backgroundColor: '#ef4444', marginRight: 6 }} />;
        } else if (date >= todayStart && date < tomorrowStart) {
            return <span title="Aujourd'hui" style={{ display: 'inline-block', width: 10, height: 10, borderRadius: '50%', backgroundColor: '#f59e0b', marginRight: 6 }} />;
        } else {
            return <span title="À venir" style={{ display: 'inline-block', width: 10, height: 10, borderRadius: '50%', backgroundColor: '#10b981', marginRight: 6 }} />;
        }
    };

    // Technician avatar with initials
    const getTechnicianAvatar = (tech: { nom: string } | null | undefined) => {
        if (!tech) return <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 28, height: 28, borderRadius: '50%', backgroundColor: '#e5e7eb', color: '#9ca3af', fontSize: 11, fontWeight: 600 }}>?</span>;
        const initials = tech.nom.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase();
        // Generate color from name
        const colors = ['#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#ec4899', '#14b8a6', '#f97316'];
        const colorIndex = tech.nom.charCodeAt(0) % colors.length;
        return (
            <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 28, height: 28, borderRadius: '50%', backgroundColor: colors[colorIndex], color: 'white', fontSize: 11, fontWeight: 600, marginRight: 6 }}>
                {initials}
            </span>
        );
    };
    ...
    const getProgressLine = (statut: string) => {
        ...
        const currentIndex = statut === 'annulee' ? -1 : steps.findIndex(s => s.key === statut);
                ...
                {steps.map((step, idx) => (
                    <div key={step.key} style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        {idx < steps.length - 1 && (
                            <div style={{
                                width: 16, height: 2,
                                backgroundColor: idx < currentIndex ? steps[idx + 1].color : '#e5e7eb'
                            }} />
                        )}
                    </div>
                ))}
    ...
    const filteredInterventions = interventions.filter((intervention) => {
        ...
        return new Date(a.datePlanifiee).getTime() - new Date(b.datePlanifiee).getTime();
    });
    ...
    const allInterventions = interventions.filter((intervention) => {
        ...
        return new Date(b.datePlanifiee).getTime() - new Date(a.datePlanifiee).getTime();
    });
        ...
        <div className="space-y-6" style={{ color: 'var(--text-primary)' }}>
            ...
            <div style={{
                ...
                <div>
                    ...
                    <p style={{ color: 'var(--text-secondary)' }}>Gestion des interventions et planning</p>
                </div>
                ...
                <div style={{
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <span style={{ color: 'var(--text-secondary)' }}> - Vous consultez les données mises en cache.</span>
                            {pendingSyncCount > 0 && (
                                <span style={{
                                    marginLeft: '8px',
                                    padding: '4px 10px',
            ...
            {!showForm && (
                <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))' }}>
                    {[
                        ...
                        { value: interventions.filter(i => i.statut === 'planifiee').length, label: 'Planifiées', color: '#3b82f6' },
                        { value: interventions.filter(i => i.statut === 'en_cours').length, label: 'En cours', color: '#f59e0b' },
                        { value: interventions.filter(i => i.statut === 'terminee').length, label: 'Terminées', color: '#10b981' }
                    ].map((stat, idx) => (
                        <div key={idx} style={{
                            backgroundColor: 'var(--bg-primary)',
                            padding: '16px',
                    ...
                    {overdueCount > 0 && (
                        <div style={{
                            ...
                            <div style={{ fontSize: '1.75rem', fontWeight: 700, color: '#ef4444' }}>{overdueCount}</div>
                            <div style={{ fontSize: '0.9rem', color: 'var(--text-secondary)' }}>⚠️ En retard</div>
                        </div>
                    )}
                </div>
            )}
                            ...
                            <table className="table">
                                ...
                                <tbody>
                                    {sortInterventions(filteredInterventions).length > 0 ? (
                                        sortInterventions(filteredInterventions).map((intervention) => (
                                            <tr
                                                key={intervention.id}
                                            ...
                                            >
                                                <td>
                                                    <div style={{ display: 'flex', alignItems: 'center' }}>
                                                        {getPriorityIndicator(intervention.datePlanifiee, intervention.statut)}
                                                ...
                                                {user?.role === 'admin' && (
                                                    <td>
                                                        <div style={{ display: 'flex', alignItems: 'center' }}>
                                                            ...
                                                            <span>{intervention.technicien?.nom || <span className="text-gray-400">Non assigné</span>}</span>
                                                        </div>
                                                    </td>
                                                )}
                                                <td>
                                        ...
                                        <tr>
                                            <td colSpan={user?.role === 'admin' ? 6 : 5}>
                                                <div className="text-center py-12">
                                                    ...
                                                    <h3 className="text-lg font-medium text-gray-900">Aucune intervention aujourd'hui</h3>
                                                    <p className="text-gray-500 mt-2">
                                                        {searchTerm ? 'Aucune intervention correspondante' : 'Utilisez le calendrier pour voir toutes les interventions'}
                                                    </p>
                                                </div>
                                            </td>
                                        </tr>
                    ...
                    <div className="fade-in">
                        <div className="mb-4 flex justify-between items-center flex-wrap gap-3">
                            <div className="flex flex-col gap-2">
                                <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)', letterSpacing: '0.3px' }}>
                                    Filtrer par statut
                                </span>
                                <div className="flex gap-2 flex-wrap">
                                    <button
                                        ...
                                        style={{
                                            ...
                                            cursor: 'pointer',
                                            transition: 'all 0.2s',
                                            backgroundColor: statusFilter === 'all' ? 'var(--text-primary)' : 'var(--bg-secondary)',
                                            color: statusFilter === 'all' ? 'var(--bg-primary)' : 'var(--text-secondary)'
                                        }}
                                    >
                                        Tous
                                    </button>
                                    <button
                                        ...
                                        style={{
                                            ...
                                            cursor: 'pointer',
                                            transition: 'all 0.2s',
                                            backgroundColor: statusFilter === 'planifiee' ? '#3b82f6' : 'rgba(59, 130, 246, 0.15)',
                                    ...
                                    <button
                                        ...
                                        style={{
                                            ...
                                            cursor: 'pointer',
                                            transition: 'all 0.2s',
                                            backgroundColor: statusFilter === 'en_cours' ? '#f59e0b' : 'rgba(245, 158, 11, 0.15)',
                                    ...
                                    <button
                                        ...
                                        style={{
                                            ...
                                            cursor: 'pointer',
                                            transition: 'all 0.2s',
                                            backgroundColor: statusFilter === 'terminee' ? '#10b981' : 'rgba(16, 185, 129, 0.15)',
                                    ...
                                    <button
                                        ...
                                        style={{
                                            ...
                                            cursor: 'pointer',
                                            transition: 'all 0.2s',
                                            backgroundColor: statusFilter === 'annulee' ? '#ef4444' : 'rgba(239, 68, 68, 0.15)',
                            ...
                            <span style={{ color: 'var(--text-secondary)', fontSize: '14px' }}>
                                {allInterventions.length} intervention{allInterventions.length !== 1 ? 's' : ''}
                            </span>
                        </div>
                        <div style={{ overflowX: 'auto' }}>
                            <table className="table">
                                    ...
                                    {sortInterventions(allInterventions).length > 0 ? (
                                        sortInterventions(allInterventions).map((intervention) => (
                                            <tr
                                                key={intervention.id}
                                                className="clickable-row"
                                                onClick={() => navigate(`/interventions/${intervention.id}`, { state: { from: 'all' } })}
                                            >
                                                <td>
                                                    <div style={{ display: 'flex', alignItems: 'center' }}>
                                                        {getPriorityIndicator(intervention.datePlanifiee, intervention.statut)}
                                        ...
                                        <tr>
                                            <td colSpan={6}>
                                                <div className="text-center py-12">
                                                    ...
                                                    <p className="text-gray-500 mt-2">
                                                        {searchTerm ? 'Aucune intervention correspondante à votre recherche' : 'Aucune intervention enregistrée'}
                                                    </p>
                                                </div>
                                            </td>
                                        </tr>
                        ...
                        <div className="fade-in">
                            ...
                            <div key={calendarKey} className={transitionClass} style={{ width: '100%', overflow: 'hidden' }}>
                                <Calendar
                                    ...
                                    onSelectEvent={(event: any) => navigate(`/interventions/${event.resource.id}`, { state: { from: 'calendar' } })}
                                />
                            </div>
                        </div>
                    )}
                                            ...
                                            <div className="selection-list">
                                                {clients
                                                    .filter(client =>
                                                        ...
                                                        (client.rue && client.rue.toLowerCase().includes(clientSearch.toLowerCase())) ||
                                                        (client.ville && client.ville.toLowerCase().includes(clientSearch.toLowerCase()))
                                                    )
                                                    .map((client) => (
                                                        <div
                                                            key={client.id}
                                                        ...
                                                        >
                                                            <div className="selection-item-info">
                                                                ...
                                                                <span className="selection-item-subtitle">
                                                                    {client.rue ? `${client.rue}, ${client.codePostal || ''} ${client.ville || ''}`.trim() : 'Sans adresse'}
                                                                </span>
                                                            </div>
                                    ...
                                    <div className="fade-in">
                                        ...
                                        <div className="form-group">
                                            ...
                                            <div className="selection-list">
                                                ...
                                                {techniciens.filter(t => t.role !== 'admin' && t.nom.toLowerCase().includes(technicianSearch.toLowerCase())).length === 0 && (
                                                    <div style={{ padding: '10px', textAlign: 'center', color: 'var(--text-secondary)' }}>
                                        ...
                                        <div className="form-group">
                                            ...
                                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                                                <div>
                                                    <label style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', marginBottom: '5px', display: 'block' }}>Date</label>
                                                    <input
                                                        type="date"
                                                        className="form-input"
                                                        value={formData.datePlanifiee ? formData.datePlanifiee.split('T')[0] : ''}
                                                        onChange={(e) => {
                                                            const date = e.target.value;
                                                            const time = formData.datePlanifiee ? formData.datePlanifiee.split('T')[1] : '09:00';
                                                            if (date) {
                                                                setFormData({ ...formData, datePlanifiee: `${date}T${time}` });
                                                            } else {
                                                ...
                                                <div>
                                                    <label style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', marginBottom: '5px', display: 'block' }}>Heure</label>
                                                    <input
                                                        ref={timeInputRef}
                                                        type="time"
                                                        className="form-input"
                                                        value={formData.datePlanifiee ? (formData.datePlanifiee.split('T')[1] || '').substring(0, 5) : ''}
                                                        onChange={(e) => {
                                                            const time = e.target.value;
                                                            const date = formData.datePlanifiee ? formData.datePlanifiee.split('T')[0] : new Date().toISOString().split('T')[0];
                                                            if (time) {
                                                                setFormData({ ...formData, datePlanifiee: `${date}T${time}` });
                                                            }
                                                        }}
                                            ...
                                            <p className="text-muted" style={{ marginTop: '10px', fontSize: '0.875rem' }}>
